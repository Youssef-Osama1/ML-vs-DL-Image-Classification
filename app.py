...
def load_models():
    models = {}
    # ── Download from Hugging Face if not already cached ──────────────────────
    HF_REPO = "itsYoussefAI/teeth-model"
    model_path = "best_resnet.h5"
...
if uploaded:
    ...
    with st.expander("📊 Full probability breakdown"):
        ...
        if model:
            ...
            rows = {
                "Class": CLASS_NAMES,
                "Full Name": [CLASS_DESCRIPTIONS[c] for c in CLASS_NAMES],
...
st.markdown("---")
st.caption(
    "Model: ResNet50 (Transfer Learning + Fine-Tuning) trained on the Teeth Dataset (7 classes, 3,087 images). "
